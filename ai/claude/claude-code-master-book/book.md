...
- [3장. AI 협업 워크플로](#3장-ai-협업-워크플로)
  - [3.1 리뷰 워크플로 — AI 협업의 골든 사이클](#31-리뷰-워크플로--ai-협업의-골든-사이클)
...
> 🎯 **3장의 핵심 질문**
>
> 클로드 코드가 일을 잘 해주는 게 아니라, **내가 클로드 코드를 잘 쓰는 법**.
> AI가 만든 결과를 **그대로 신뢰하지 않고**, 어떻게 검증·반복하며 품질을 끌어올릴 것인가?

---

### 3.1 리뷰 워크플로 — AI 협업의 골든 사이클

#### 3.1.1 왜 워크플로가 중요한가

> 🚨 **흔한 실수**: "AI가 짠 코드 → 그냥 커밋 → 일주일 뒤 의문의 버그"
>
> AI는 **"그럴듯한 코드"**를 빠르게 생성하지만, **"맞는 코드"**를 보장하진 않는다.
> 검증·테스트·되돌리기를 **루프 안에 박아넣어야** 안정적으로 협업 가능.

핵심 비유:

- ❌ AI = **자동 완성 머신** → 결과만 받아서 쓴다 (위험)
- ✅ AI = **인턴 개발자** → PR을 받아서 **리뷰하고 머지**한다 (안전)

#### 3.1.2 골든 사이클 — 6단계 순환 구조

```mermaid
flowchart TD
    Start([🟢 시작])
    S1[1️⃣ 작은 작업 요청<br/>한 번에 하나만<br/>명확한 완료 조건<br/>필요한 컨텍스트 전달]
    S2[2️⃣ AI 결과물 수령]
    S3[3️⃣ 코드 리뷰<br/>이해 + 검증<br/>요구사항 충족 확인<br/>코드 품질 검토]
    S4[4️⃣ 테스트 실행]
    Q1{문제 발견?}
    S5a[5a 수정 요청 또는 되돌리기<br/>구체적 수정사항 전달<br/>필요 시 git revert]
    S5b[5b 커밋<br/>의미 있는 메시지<br/>작은 단위로]
    Q2{다음 작업?}
    Done([🏁 개발 완료])
...
##### 1️⃣ 작은 작업 요청 — 골든 룰 3가지

> 💡 **핵심**: "큰 덩어리 1개" ❌ → "**잘게 쪼갠 작은 단위 N개**" ✅

| 룰 | 설명 | 좋은 예 |
|----|------|---------|
| 🔴 한 번에 하나 | 여러 기능 동시에 X | "로그인 폼 컴포넌트만 만들어줘" |
| 🔴 명확한 완료 조건 | "잘 만들어줘" 같은 모호한 요청 X | "이메일/패스워드 검증 + 빈 값 에러 메시지 표시" |
| 🟡 필요한 컨텍스트 | 클로드가 모르는 정보 미리 제공 | "@src/api/auth.ts 이 API 응답 구조에 맞춰서" |
...
→ 디자인·검증·API 연동·에러 처리·라우팅까지 한꺼번에 → 어디가 틀렸는지 추적 불가

✅ **좋은 예**:

```text
> @src/types/auth.ts 의 LoginRequest 타입을 사용해서
> 이메일/패스워드 입력 폼 컴포넌트(LoginForm.tsx)를 만들어줘.
> 완료 조건:
>   1. 이메일 형식 유효성 검증 (zod 사용)
>   2. 빈 값일 때 에러 메시지 빨간색으로 표시
>   3. submit 핸들러는 props로 받음 (API 호출은 부모에서)
> 스타일은 Tailwind만 사용. 라우팅은 이번 작업에 포함 X.
```

→ 검증 가능한 단위로 쪼개짐 → 리뷰·되돌리기 쉬움

##### 2️⃣ AI 결과물 수령 — "받았다"가 끝이 아님

- 🟡 결과물이 **여러 파일**에 걸쳐 있다면 → **각 파일 변경사항 한 번씩 훑기**
- 🔴 **숨겨진 부수 효과 확인** — 클로드가 요청 외 파일을 건드렸을 수 있음
- 💡 `git diff`로 **전체 변경 범위** 먼저 파악

```bash
> !git status     # 어떤 파일이 변경됐는지
> !git diff       # 무엇을 어떻게 바꿨는지
```

##### 3️⃣ 코드 리뷰 — 이해 + 검증

> 🚫 **"코드를 읽지 않고 커밋"은 AI 협업의 가장 큰 적**.
>
> 1년 뒤 버그 났을 때 **본인이 못 고치는 코드**가 됨.

리뷰 체크리스트:

- ✅ **이해**: 한 줄 한 줄 무슨 일을 하는지 **내가 설명할 수 있는가**?
- ✅ **요구사항 충족**: 1️⃣단계의 완료 조건 **모두 만족**하는가?
- ✅ **코드 품질**:
  - 함수 너무 길지 않은가? (30줄 이상이면 의심)
  - 중복 로직 없는가?
  - 네이밍 명확한가?
  - 타입 안정성? (`any` 남용 X)
  - 기존 패턴/컨벤션 따랐는가?
- ✅ **부수 효과**: 의도치 않은 파일 수정·`@import`·`console.log` 잔존?

> 💡 클로드에게 **자가 리뷰 요청**하기:
>
> ```text
> > 방금 만든 LoginForm.tsx를 다시 읽고 다음 관점에서 리뷰해줘:
> > 1. 타입 안정성  2. 에러 처리 누락  3. 컴포넌트 재사용성
> ```

##### 4️⃣ 테스트 실행 — 자동화된 검증

리뷰가 끝났다면 **반드시 실제로 돌려본다**.

| 테스트 유형      | 언제                                         |
| ---------------- | -------------------------------------------- |
| 타입 체크        | TypeScript 변경 시 매번 (`tsc --noEmit`)     |
| 린트             | 매번 (Hook으로 자동화 가능)                  |
| 단위 테스트       | 로직 변경 시 (`npm test -- --related`)        |
| **수동 동작 확인** | UI 변경 시 **브라우저에서 직접 클릭** 필수    |

> ⚠️ **타입 체크와 테스트가 통과한다 ≠ 기능이 맞다**.
> UI는 반드시 **눈으로 확인**, API는 **실제 요청 + 응답** 확인.

##### 5a 문제 발견 → 수정 요청 또는 되돌리기
...
| ----------------------------------- | ------------------ |
| 작은 수정 1~2곳                     | 수정 요청          |
| 접근 자체가 잘못됨                  | **`git revert`** → 다시 요청 |
| 같은 부분을 **3번 이상 고쳐도** 안 됨 | 멈추고 접근 재검토 |

✅ **좋은 수정 요청**:

```text
> LoginForm.tsx의 handleSubmit에서 catch 블록이 빠졌어.
> 네트워크 에러 났을 때 setError("네트워크 오류") 호출하도록 추가해줘.
```
...
→ 클로드가 "어디가" "왜" 이상한지 모름 → 같은 실수 반복

##### 5b 커밋 — 작게, 의미있게

```bash
> !git add src/components/LoginForm.tsx
> !git commit -m "feat: LoginForm 컴포넌트 - 이메일/패스워드 검증 추가"
```

- 🔴 **한 작업 = 한 커밋** — 나중에 `git revert` 가능하도록
- 🟡 **메시지는 "왜"** — "Update file" ❌ / "feat: 로그인 검증 추가" ✅
- 🟢 **AI 도움 받았으면 footer에 명시** (팀 정책에 따라):
  ```text
...
```text
❌ "전체 인증 시스템 구현해줘 (로그인/회원가입/비밀번호 재설정/세션 관리)"
   → 결과물 1500줄 → 리뷰 불가능 → 그냥 커밋 → 나중에 폭탄
...
```text
❌ 5개 작업 연속으로 받아서 한꺼번에 테스트
   → 어떤 작업이 깨뜨렸는지 추적 어려움 → 시간 2배

✅ 작업마다 테스트 → 통과 → 커밋 → 다음 작업
   → 문제 즉시 발견, git bisect 필요 없음
```

##### 안티 패턴 3: "이상하면 그냥 다시 해줘"

```text
❌ "다시 해줘" → 클로드가 다른 방향으로 또 틀림 → 또 "다시" → 무한 루프

✅ 첫 시도가 잘못됐으면:
   - 무엇이 잘못됐는지 한 줄로 정의
   - git revert로 깨끗한 상태 복원
   - 요구사항을 더 구체적으로 다시 작성
```

##### 안티 패턴 4: "커밋 안 하고 계속 진행"

```text
❌ 5개 작업 완료될 때까지 커밋 안 함
   → 4번째에서 망가짐 → 1~4 모두 한꺼번에 손댐 → revert 불가

✅ 1️⃣작업 → 검증 → 커밋 → 2️⃣작업 → 검증 → 커밋 ...
   → 언제든 안전한 지점으로 되돌릴 수 있음
```
...
- [ ] 이 작업, **30분 안에** 끝낼 수 있을 만큼 작은가?
- [ ] **완료 조건**을 한 문단으로 적을 수 있는가?
- [ ] 클로드가 모르는 컨텍스트(파일·타입·API)를 `@`로 첨부했는가?

작업 진행 중 자문:

- [ ] AI 결과를 **읽고 이해**했는가?
- [ ] **테스트**를 실제로 돌렸는가? (눈으로 확인 포함)
- [ ] 변경 범위가 요청한 만큼인가? (`git diff`)

작업 완료 후 자문:

- [ ] **의미 있는 메시지**로 커밋했는가?
- [ ] 다음 작업도 **같은 사이클**로 진행할 준비가 됐는가?

#### 3.1.6 사이클 속도 ↑↑ 만드는 자동화

이 워크플로의 반복 작업을 **앞 장에서 배운 도구로 자동화**할 수 있다.

| 단계        | 자동화 도구                                    |
| ----------- | ---------------------------------------------- |
| 1️⃣ 컨텍스트 | **CLAUDE.md** (프로젝트 정보 자동 로드)        |
| 3️⃣ 리뷰     | **`code-reviewer` Skill** (`/code-reviewer`)   |
| 4️⃣ 테스트   | **PostToolUse Hook** (`format-ts.sh`, `lint-ts.sh`, `test-ts.sh`) |
| 5a 차단     | **PreToolUse Hook** (`block-dangerous.sh`)     |
| 5b 알림     | **Stop Hook** (Slack/Telegram 통보)            |

> 🎯 **결국 골든 사이클의 본질**: 사람은 **판단**, AI는 **생성**, 도구는 **반복 검증**.
> 셋의 역할 분리가 안정적인 협업의 핵심.

#### 🎯 3.1 핵심 요약

- **AI는 자동 완성이 아니라 인턴 개발자** — 받은 PR을 리뷰해서 머지하는 마인드
- **6단계 사이클**: 작은 요청 → 결과 수령 → 리뷰 → 테스트 → (수정/커밋) → 다음
- **작은 작업·명확한 완료 조건·필요 컨텍스트** 3종 세트가 1단계의 핵심
- **읽지 않고 커밋 금지** — 1년 뒤 본인이 못 고치는 코드는 부채
- **작업당 1커밋** — 언제든 `git revert` 가능한 안전선
- **3회 이상 같은 실수면 멈추고 재검토** — 무한 루프 방지
- 반복 검증은 **Skill / Hook**으로 자동화 → 사람은 판단에만 집중
